...
* The xref:./provider-agreement.adoc[DKE application provider agreement] has to be signed.
* The xref:./marketplace.adoc[marketplace entry] has to be created with a "Coming soon" label.
...
|xref:./integration/reonboarding.adoc[Reonboarding]
|Always
a|
...
|xref:./commands/endpoint.adoc#subscribtion-command[dke:subscription]
...
| xref:./tmt/taskdata.adoc[TaskData] and link:./tmt/shape.adoc[Shape]
a|
