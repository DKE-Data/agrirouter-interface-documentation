...
This is mainly useful when using an MQTT router deivce, to recognize if an endpoint has been deleted by the user from the agrirouter UI. All other cases can be known to the application itself.
...
The result is a simple `ACK` (acompanied with `response_code == 201`) or an `ACK_WITH_FAILURE` (`response_code == 404`).
...
Receiving an `ACK_WITH_FAILURE` means that the endpoint has recently been deleted.
