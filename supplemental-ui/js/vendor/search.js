...
window.antoraLunr = (function (lunr) {
  ...
  function highlightText(doc, position) {
    ...
    var contextAfter =
      end + contextOffset > textEnd ? textEnd : end + contextOffset;
    ...
    if (start === 0 && end === textEnd) {
      ...
      hits.push(
        document.createTextNode(
          "..." + text.substr(contextBefore, start - contextBefore)
        )
      );
      hits.push(highlightSpan);
      hits.push(
        document.createTextNode(text.substr(end, contextAfter - end) + "...")
      );
    }
    return hits;
  }
