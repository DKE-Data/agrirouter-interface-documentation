...
All pages will be the result of one single GET Request, if all messages are delivered to the outbox, when the endpoint requests them. If not all messages have been delivered from feed to the outbox at this point in time, the endpoint will have to poll for the messages(Make sure to meet the link:../messaging-workflow.adoc#request-sending-frequency[request frequency requirements]). In MQTT, all Pages are delivered one after each other.
...
== Metadata
From agrirouter Release 1.2, messages can be extended by metadata to provide further information.
