...
This process can be performed in parallel to the link:./general-integration-process.adoc[integration process].
...
After certification process is finalized, the app provider may remove the "coming soon". In general, the application or hardware provider is responsible for correctness and for keeping their solution information on the marketplace up to date. The login to the marketplace can be found link:https://my-agrirouter.com/en/interner-bereich/my agrirouter/choose maintance/[here]. Changes are reviewed before activation to make avoid changes without certification.
