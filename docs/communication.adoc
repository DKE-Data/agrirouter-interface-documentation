...
REST uses HTTP requests, that results in a HTTP Response.
...
The agrirouter provides a MQTT Broker serverside, so, an app instance has to connect to this server with its client.
...
A MQTT connection requires 2 types of certificates. On the one hand, there is a MQTT Server Certificate, for which you can find the keys link:./keys.adoc#MQTT-communication[here].

This is required to setup a MQTT Connection over SSL.
...
==== Router Devices
When using MQTT for a telemetry platform or a farming software with multiple accounts, multiple MQTT connections to different brokers would be required. To avoid this, agrirouter introduced **router devices** which allows to use just one connection for multiple, virtual endpoints.
