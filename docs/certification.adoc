...
|====
|Topic |Description |Expected results / Acceptance criteria
|Polling |It is checked, if your application does not flood agrirouter with polls |The application shall not poll the outbox of each app instance more often than described link:./messaging-workflow.adoc#request-sending-frequency[here].
|====
